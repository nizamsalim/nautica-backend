...
export const predictSuitability = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const normalizationRanges = Object.values({
      ...
      wind_speed: { min: 0, max: 100 }, // wind speed range in kph
      ...
      gust_speed: { min: 0, max: 100 }, // gust speed in kph
    ...
    const weights = Object.values({
      cloud: 2, // Direct relationship
      uv: -3, // Inverse relationship
      temp_land: 2, // Direct relationship
      temp_water: 2, // Direct relationship
      wind_speed: -3, // Inverse relationship
      pressure: 1, // Neutral relationship
      humidity: -2, // Inverse relationship
      sig_ht: -3, // Inverse relationship
      swell_ht: -3, // Inverse relationship
      gust_speed: -2, // Inverse relationship
      wind_chill: -2, // Inverse relationship
      dew_point: -2, // Inverse relationship
...
export const getNormalizationRanges = async (req: Request, res: Response) => {
  ...
  const long = 80.2824;
  const time = 17;
  const api = `${HISTORY_API}q=${lat}%2C${long}&hour=${time}&dt=`;
...
const getHistoryData = (
  api: string,
  date: string
): Promise<HistoryWeatherData> => {
  return new Promise(async (resolve, reject) => {
    try {
      const r = await axios.get(`${api}${date}`);
      // res.json({ data: r.data });
      const data = r.data as WeatherDataResponse;
      const hourlyForecast = data.forecast.forecastday[0].hour[0];
