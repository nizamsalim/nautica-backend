import { ForecastDay } from "../constants/WeatherResponseInterface";

export default function assignRanges(dailyForecasts:ForecastDay[]) {
    let ranges = {
    ...
    for (let i = 0; i < dailyForecasts.length; i++) {
      ...
      if (hour.sig_ht_mt! < ranges.sig_ht.min) {
        ranges.sig_ht.min = hour.sig_ht_mt as number;
      } else if (hour.sig_ht_mt! > ranges.sig_ht.min) {
        ranges.sig_ht.max = hour.sig_ht_mt as number;
      }
  
      if (hour.swell_ht_mt! < ranges.swell_ht.min) {
        ranges.swell_ht.min = hour.swell_ht_mt as number;
      } else if (hour.swell_ht_mt! > ranges.swell_ht.min) {
        ranges.swell_ht.max = hour.swell_ht_mt as number;
      }
  
      if (hour.gust_kph < ranges.gust_speed.min) {
        ranges.gust_speed.min = hour.gust_kph as number;
      } else if (hour.gust_kph > ranges.gust_speed.min) {
        ranges.gust_speed.max = hour.gust_kph as number;
      }
